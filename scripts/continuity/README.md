...
Fixture, evidence, startup-packet, and generator versions are `1.3.2`. Required provenance includes a structurally valid primary live GitHub repository name, a primary nonempty live default-branch name and exact lowercase target SHA, public-safe root and normalized origin identity, plus binding of the supplied run to workflow `ContractorOS Control Gates` with database ID `309083557`, the `pull_request` event, the exact PR head SHA and branch, the `contractoros-control-gates` job, and the PR check link for that exact run. Missing or malformed live repository/default-branch values block as unavailable evidence; a valid different repository name reaches identity comparison and quarantines. No live repository identity falls back to CLI arguments, local Git, canonical state, or conventional branch names.

Before comparison, rendering, or output creation, the `1.3.2` runtime contract validates exact repository, issue, pull-request, check, workflow-run, job, step, marker, auto-merge and optional source-command shapes with bounded list sizes and no unknown nested properties. Malformed fixture/evidence input returns exit `5` without a traceback; unavailable or malformed authoritative live responses remain exit `3`.
