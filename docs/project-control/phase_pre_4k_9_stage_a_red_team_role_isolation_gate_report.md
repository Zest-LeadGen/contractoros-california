...
Version `1.4.0` makes actor evidence mandatory in newly generated packets. Program direction is not actor authority. Repository, issue, pull request, branch, exact SHA, lifecycle, authority-source scope, observation time, actor actions, and next authorized actor are deterministically bound.
...
- `continuity-evidence.json`: `9003a9006505d8b9f2b5b116a732e2b7389d40cb52d5ed00b41d02a5b81766a5`
- `RED_TEAM_STARTUP_PACKET.md`: `0db4eacd51b75212e3079986005a27c5302ecb318e0ab6f90b4f3c6591f6958c`
- recorded and independently recomputed packet hash: `c71af858fc5852cf00426db31481816b2d42d5ab340043d0001619f85737b1e4`

Changing only `actor_contract.PROGRAM_NEXT_ACTION` changed the packet hash to `26c91bdcee4059ce04b4a4e04449363a65d68f8e8355f8c27c82e710cded8779`, proving actor-contract material participates in hashing.
...
Packet 3 actor-contract integration tests passed 7 test methods.

## Existing Continuity Regression Evidence

The full continuity suite passed 383 tests after the `1.4.0` migration. Existing Issue #49 classifications and failure precedence remain intact.
...
| `state/contractoros-state.yaml` | Bind canonical observed state to verified main and active Issue #55 before PR creation. | Live preflight | Current main, gate, issue, null PR, lifecycle, blockers, evidence IDs, and references. | NO |
...
The final rerun passed the six-case role self-test, all 40 dedicated role-contract test methods, all 7 focused actor-integration test methods within the full suite, and all 383 continuity tests. Both schemas parse as JSON. Fixed-time generated outputs compare byte for byte, independent hash recomputation agrees, and an actor-contract mutation changes the hash.
...
Source and command verified within reported local Stage A tests only; external exact-SHA review and human approval are pending, the PR will remain unmerged, and full runtime isolation is not proven.
...
Pending external exact-SHA red-team review. No `RED_TEAM_DECISION` marker is developer-authored or present.

## Human Approval Status

Pending; zero human approvals are claimed.
...
After the developer PR handoff, `EXTERNAL_EXACT_SHA_RED_TEAM` is the only next authorized actor in scope. Until then, the bounded developer may finish Issue #55 validation and PR preparation only.
