...
- Delivered: .github/dependabot.yml (npm ×2 app dirs + github-actions, weekly, limit 2; advisory-signal posture recorded in-file — Dependabot PRs cannot pass the wall or merge; real changes ride authorized dependency-lane PRs). Dependency-review workflow RE-ADDED (dropped c78dcfe as premature; Dependency Graph now verified enabled) — SHA-pinned from birth (a1d282b3, v5.0.0 resolved live), minimal permissions, timeout, concurrency, persist-credentials:false. Private-data/credential-pattern scan EXECUTED over all 258 tracked files (nine pattern classes, 19:04:50Z): one raw hit, zero real findings — the hit is the deliberate negative-test fixture test_embedded_remote_credentials_are_rejected proving the collector rejects embedded credentials; classified, not suppressed.
