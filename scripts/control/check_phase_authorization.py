...
def main():
    ...
    rc, listing = git(["ls-tree", "-r", "--name-only", f"origin/{base_ref}", PA_DIR + "/"])
    if rc != 0:
        return fail_env(f"cannot list {PA_DIR} at origin/{base_ref}")
    candidates = []
    ...
    permit_path, permit = candidates[0]
    ...
    for path, kind in changed:
        if path.startswith(PA_DIR + "/") and (path, "add") != (permit_path, "add"):
            failures.append(f"AUTHORIZATION_SELF_MODIFICATION_DENIED: {path}")
        if path == permit_path and kind != "add":
