...
C3.3 is the final authority-binding correction required by external whole-PR review `4680891490` at exact head `09243095e3224db872f8c0b843ba19716abdec8b`. It changes only the four reviewed gaps: active-family local-head/default-branch/PR/run binding, closed-gate local/main/merge binding without conflating the pre-merge PR head, nullable authoritative live PR/workflow field presence and auto-merge normalization, decisive-review RFC3339 timestamp validation, and canonical scalar lifecycle/consistency validation and comparison. Fixture, evidence, packet and generator contracts migrate coherently to `1.3.3`. It adds 19 C3.3 tests and passes 267 total tests locally. Two fixed-time consistent-fixture generations are byte-identical and the independent packet hash matches `bab6c5137aadc344f23cb0651794d9b02414b9a4ea331f445b7571e32a7137b6`; both schemas parse. Four separate offline adversarial fixtures return source-binding quarantine exit `2`, completed-workflow-null-conclusion exit `5`, invalid-review-timestamp exit `5` without traceback, and stale canonical-consistency exit `2`. At `2026-07-12T23:00:00Z`, the required live collector used Issue #49, PR #50, run `29211199001`, and canonical ref `09243095e3224db872f8c0b843ba19716abdec8b`; it returned exit `0`, `requires_live_verification`, missing marker, zero qualifying approvals, inactive auto-merge, and packet hash `a88e787144f16dbdca306ff8cab8d801299cbd521cbfcb4ee28c8b7877b5e00f`. All eleven local controls, all three live-body pull-request-context reproductions, and both diff checks pass without workflow or control-script edits. Commit, push, exact-new-head workflow inspection and fresh exact-SHA review remain required; no marker, approval, merge, closeout, Phase 4K-9 or Phase 4I action is authorized.

C3.4 is the bounded fail-closed correction required by external audit record `4680930560` against C3.3 starting head `5fb3f33ec25da29e76786ce663a88d0a0942d38c`. It corrects six findings only: response identity is bound to the requested permission candidate under a canonical case-fold rule; live collection proves a clean unchanged worktree with public-safe hashes; every normalized review family is structurally validated before approval reduction; decisive review ordering uses UTC instants; non-null live Issue `closedAt` values must be RFC3339; and closed-gate PR base binds to the verified default branch. Fixture, evidence, packet and generator contracts migrate coherently to `1.3.4`. Local focused validation adds 13 C3.4 adversarial tests and the complete suite currently passes 280 tests. Two fixed-time fixture generations are byte-identical, the independent packet hash matches `13e2927970f1b6176b33549ebe765c834355eb355b8e97c9283d4218a38e65f7`, and both schemas parse. Commit, push, exact-new-head workflow inspection, outside-repository live collection and fresh external whole-PR exact-SHA review remain required; no marker, approval, merge, closeout, Phase 4K-9 or Phase 4I action is authorized.
...
## C3.5 Developer Correction Packet

C3.5 corrects four external-review findings only: it stages collected four-field review evidence before seven-field evaluation, uses the explicit all-untracked status command and equal before/after exact HEAD proof, validates every derived approval claim against deterministic reduction with disjoint/reason-complete identities, and binds live plus normalized issue closeout state to `closedAt`. The C3.5 contract was `1.3.5`. Eleven focused C3.5 tests and the complete 291-test suite passed locally; two fixed-time generations were byte-identical, both schemas parsed, and the independently recomputed packet hash was `f1aa0c3bfe76d8d063579a5019f748bff1514fa6bdda94ea47ed660763415989`. Local and pull-request-context controls passed with only the 17 authorized paths changed; workflow and control scripts remained unchanged. This packet did not add a `RED_TEAM_DECISION`, approval, merge, issue closeout, Phase 4K-9 action, or Phase 4I resumption.

## C3.6 Developer Correction Packet

C3.6 corrects only the external whole-PR findings at `e91a9afab31bbee037b4d05f0cd5f51dd0cba27f`: exact invocation-bound GitHub command vectors, collision-safe canonical reason identities, schema/runtime `_evidence` equivalence, and mathematically derived source-command bounds. The fixture, evidence schema, packet schema, and generator contract migrate coherently to `1.3.6`. The developer-local suite contains 297 tests, including exact command rejection, successful mocked live command acceptance, case-only reason-key collisions, collision-free deterministic maps, standard-library schema-reason assertions, zero/one/maximum permission candidates, and source-command maximum/plus-one bounds. Two fixed-time generations are byte-identical, both schemas parse, and independent packet-hash recomputation matches `8755f33d6416160ad16af824ead9e53d3a3a183f8f823f4546721ce7c7ea75d0`. All required local controls, both diff checks, and all three PR-context reproductions pass without workflow or control-script modification. No workflow, `scripts/control/**`, dependency, lockfile, product, runtime, backend, database, build, deployment, release, marker, review, approval, merge, issue-closeout, Phase 4K-9, Phase 4I, or Issue #53 change is included. Commit, push, exact-head workflow inspection, outside-repository live collection, PR-body reconciliation, and external whole-PR exact-SHA review remain required.

## C3.7 Developer Correction Packet

C3.7 corrects two bounded external-review findings at starting head `b0e0231f8a2ba57cbd028ac53010035f35eb57bc`. Reviewer reduction now evaluates every reviewer's latest submitted exact-current-head decisive state, keeps unresolved `CHANGES_REQUESTED` adverse through later comments and other reviewers' approvals, clears only on that reviewer's later current-head approval or dismissal, preserves stale-head change requests as nonblocking normalized evidence, and forbids adverse aggregate decisions from supporting merge-ready or closed-gate consistency. Nonempty source-command histories now require zero return codes and exact membership in the evidence-derived read-only firewall, including fixed repository, Issue #49, PR #50, run, canonical ref, field order, pages 1–5, and permission identities sourced from normalized current-head approvals. The current fixture, evidence, packet and generator contracts migrate coherently to `1.3.7`; C3.6 history remains `1.3.6`. No review body is stored or surfaced. Sixty-eight focused C3.7 plus C3.6 regression tests and all 329 continuity tests pass. Fixed-time outputs are byte-identical, both schemas parse, the independently recomputed hash matches `40667c04edb9bce8d4c3aae4efae2f3f9c8d782261707db4359171ea540be4d8`, fixture exits match `0/0/2/2/3/4/5`, and all eleven local controls, three PR-context reproductions, and both diff checks pass. Exact-head workflow and outside-repository live results remain pending after commit and push. No workflow, `scripts/control/**`, marker, approval, merge, issue closeout, Issue #53 or #54 implementation, Phase 4K-9 start, or Phase 4I resumption is authorized.

## C3.8 Developer Correction Packet

C3.8 corrects only the three external-review findings at starting head `20cde9ac7d8a206c07e3b3c9ca27e1964b5aeb6d`. Complete collected or evaluated review evidence now validates before semantic source-command derivation can dereference reviewer records. Every fixture and live output requires an exact non-null lowercase 40-character canonical source ref in runtime, both schemas and packet rendering without changing nullable merge fields. Persisted command results now require runtime integer zero and evidence-schema `const: 0`. The current fixture, evidence, packet and generator contracts migrate coherently to `1.3.8`; C3.6 and C3.7 history remains unchanged. Seven focused C3.8 tests and all 336 continuity tests pass. Fixed-time evidence and packets are byte-identical, both schemas parse, the independently recomputed hash matches `733034ae5d75ad54065da906d11f01ac6e461df09b81623357b00833dc3ff266`, and fixture exits match `0/0/2/2/3/4/5`. All eleven local controls, all three PR-context reproductions and both diff checks pass with exactly the 17 authorized correction paths and a cumulative PR file count of 32. The first local forbidden-scope scan identified new reviewer-identity literals and one documentation line; validator-safe scope labels corrected those findings without changing the validator, workflow, or behavior. No workflow, `scripts/control/**`, dependency, lockfile, marker, review, approval, merge, issue closeout, Issue #53 or #54 implementation, Phase 4K-9 start, or Phase 4I resumption is included. Exact-head workflow inspection and outside-repository live collection remain required after the single normal commit and push.
