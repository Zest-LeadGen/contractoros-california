...
- **PS (Protect Software):** protected main (ruleset, owner-only merge); default-deny path wall + from-main copy; exact pins + digest-bound lockfiles; secret scanning + push protection ENABLED per owner read-back evidence (issue-137-comment-5245222558; the executor cannot independently re-read the admin-only settings surface — narrowed L-1 residual, disclosed); PVR ENABLED (world-readable endpoint, independently re-verifiable); no secrets in tree (H7A-3 scan clean).
...
8. LICENSE decision and repository license file (#65 required control): OPEN by owner decision state — #71 selected option B (options packet first) with LICENSE_ADDITION=NOT_AUTHORIZED; the packet remains available on owner request. Recorded here after the Opus round-1 completeness check found this item missing from the list (it was recorded at H7A-1/H7A-2 but omitted from this closeout's remainder list — the omission is owned, not defended).

## 8. Vulnerability and incident exercises (#65 required evidence — added after the Opus round-1 completeness check found this item neither delivered nor routed; the gap is owned)

Both exercises below are DESK/TABLETOP exercises executed 2026-08-10 by the executor against the committed policy (INCIDENT_RESPONSE_AND_VULNERABILITY_TRIAGE_POLICY.md); no live system, credential, or setting was touched. Honest label: documented walkthroughs, not live drills.

**8.1 Vulnerability triage exercise.** Lived case: R-DEP-SEC-001 exercised the full §1 path in production during H6-H7A — intake (Dependabot alerts), severity carried from source (2× HIGH, 1 MEDIUM), disposition ACCEPT with owner approval, dated revisits executed at every phase boundary since (07:51Z, 08:56:39Z, 21:50:48Z reads), no silent carry. Simulated case walked on paper: a NEW critical alert in apps/web — §1 clock starts at intake; FIX path = authorized dependency-lane PR with exact pin + lockfile regeneration + digest gate; if no patch exists (the image-size precedent), ACCEPT requires owner-approved dated acceptance with revisit trigger; MITIGATE requires recorded compensating control. Gap found by the exercise and recorded: the 7-day disposition clock has no machine reminder — tracking is manual until a scheduled check exists (candidate: H8 automation or the weekly CodeQL cron pattern).

**8.2 Incident response tabletop.** Scenario: suspected exposure of the executor token (the program's T10 threat). Walked §2/§3: (1) CONTAIN — owner rotates the executor token (owner act; precedent exists: the 2026-08-10 amkb80 credential logout, owner questionnaire Q3, verified absent from keyring); (2) VERIFY STATE LIVE — re-read main history, open PRs, workflow runs, PA chain from GitHub (the exact reads this program performs routinely); (3) ASSESS — reachable surface bounded by recorded facts: push-only scope, owner-only merge, armed wall, required checks — a compromised executor can push branches and file misleading evidence but cannot merge or pass the wall without a PA; (4) RECOVER via the normal gated route; (5) RECORD — dated DECISION_LOG incident entry + RISK_REGISTER residual. Gap found and recorded: rotation depends on owner availability (single-human program); no automated token-revocation path exists — accepted structural residual under H2-WAIVER-001, revisited at H9.

Routed item 9 (added in round 2 with the Opus advisory): the §8 exercises are DESK-ONLY by necessity today — re-run as LIVE drills (actual token rotation under time pressure; real alert-to-fix cycle) when H10 introduces deployed systems; recorded so the desk-form limitation cannot silently stand in for live capability.
