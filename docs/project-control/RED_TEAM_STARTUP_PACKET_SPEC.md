...
Live actor routing derives `EXTERNAL_EXACT_SHA_REVIEW`, `HUMAN_APPROVAL_PENDING`, and `MERGE_PENDING` from those three active states. A merged PR whose merge commit is not yet verified on live main uses `MERGED_MAIN_VERIFICATION_PENDING`; verified main with the linked issue still open uses `VERIFIED_MAIN_ISSUE_CLOSEOUT_PENDING`. Both verification-only states set `NEXT_AUTHORIZED_ACTOR=NONE` and every actor-specific action to `NONE`. Merge does not prove verified main, and verified main does not authorize issue closeout.
