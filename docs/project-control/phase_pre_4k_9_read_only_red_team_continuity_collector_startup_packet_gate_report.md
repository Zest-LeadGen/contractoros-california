...
C3.5 corrects four external-review findings only: it stages collected four-field review evidence before seven-field evaluation, uses the explicit all-untracked status command and equal before/after exact HEAD proof, validates every derived approval claim against deterministic reduction with disjoint/reason-complete identities, and binds live plus normalized issue closeout state to `closedAt`. The contract migrates coherently to `1.3.5`. Eleven focused C3.5 tests and the complete 291-test suite pass locally; two fixed-time generations are byte-identical, both schemas parse, and the independently recomputed packet hash is `f1aa0c3bfe76d8d063579a5019f748bff1514fa6bdda94ea47ed660763415989`. Local and pull-request-context controls pass with only the 17 authorized paths changed; workflow and control scripts remain unchanged. This packet does not add a `RED_TEAM_DECISION`, approval, merge, issue closeout, Phase 4K-9 action, or Phase 4I resumption. Commit, push, exact-new-head workflow inspection, outside-repository live collection, PR-body reconciliation, and external whole-PR exact-SHA review remain required.
