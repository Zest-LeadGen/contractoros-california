...
The version `1.3.3` scope-bound evidence contract binds active-family classification to collected local HEAD equals PR head, repository default branch and PR base equal `main`, and run SHA/branch equals the PR. Closed-gate classification binds collected local HEAD, local main, and merge commit to verified live main without conflating that post-merge SHA with the pre-merge PR head. Nullable authoritative PR keys are presence-bound; only a null auto-merge request is inactive. Completed live runs, jobs, and steps require a nonempty conclusion. Decisive review timestamps must be valid RFC3339 and non-null before selection. Canonical schema version and lifecycle state are nonempty strings, and canonical lifecycle/consistency values are compared against the active or closed family. The generator and both generated schemas use `1.3.3`; every fixture uses `fixture_version: 1.3.3`. Absolute repository paths and raw remote URLs are never persisted.
...
The collector persists only bounded public-safe review identity/state/binding fields and bounded permission identity/role fields. Before normalization, each permission response must be an object with a nonempty calculated permission, a present string role name, a user object and supported account type. Missing or inaccessible review or permission evidence is blocked with exit `3`; command-shape rejection remains exit `5` and unsafe evidence remains exit `4`. A full fifth review page is truncated and blocked. External exact-SHA red-team evidence and qualifying human approval remain separate categories; neither substitutes for the other.
