# H1-B3 Push-Context FP Remediation Gate Report
...
H1 Issue #102 — Observation follow-through: remediate the push-context contract-check false positive found at B4 intake
...
Observation-mode lineage under the merged OD-015 record (Issue #102 authorization 5229225607): a false-positive class found at the B4 intake re-measurement must be remediated before cutover. This report delivers that bounded remediation; it grants no B4 start or closeout authority.
...
- `.github/workflows/control-gates.yml` (one step gains an event condition)
- `docs/project-control/DEVELOPMENT_LEDGER.md`
- `docs/project-control/DECISION_LOG.md`
- `docs/project-control/RISK_REGISTER.md`
- `docs/project-control/phase_h1_b3_push_context_fp_remediation_report.md` (new)

No other path is changed.

## Commands Run

- Read-only scope: enumerated every push-event control-gates run on main (28 failures since 2026-07-09, all from the PR-contract step); confirmed the failing step lacked the pull_request condition its sibling steps already carry.
...
The decision log records the B4-intake finding and remediation; the risk register adds R-B3-002 (alarm-fatigue risk of a permanently red main) with its green-main-push resolution condition.

## Workflow Validation

`control-gates.yml` changes by exactly one added `if: github.event_name == 'pull_request'` line, matching the existing pattern on the marker and owner-trigger steps. Push runs now execute only push-appropriate checks; pull-request runs are byte-identical in behavior. Permissions, triggers, and all other steps unchanged.

## Security Hardening

No permission, secret, trigger, or action change; the diff is a single event condition on an existing step. The remediation narrows when a check runs; it removes no check from any pull request.
...
Adds `R-B3-002` (push-context false positive / alarm fatigue), resolving on the first green main-push run after merge.

## Decision Log Impact

Adds the B4-Intake Re-Measurement Finding section (FP classification and bounded remediation).
