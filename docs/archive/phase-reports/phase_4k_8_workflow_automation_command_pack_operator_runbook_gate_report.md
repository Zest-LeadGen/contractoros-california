...
The starting-state documentation now requires the local `main` ref to equal the approved starting SHA before any remote-reference refresh, requires both local `main` and refreshed `origin/main` to equal that SHA before any branch switch, and repeats clean-state, branch, and SHA assertions after switching. The runbook and command pack use the same fail-closed order. This correction changes documentation only and adds no review marker, approval, merge permission, or next-phase permission.

During the current three-file correction, `check_changed_files.py` passed and the immediately following forbidden-scope scan stopped the sequence on two newly added explanatory documentation lines that used a scanner-controlled remote-sync keyword outside explicit documentation context. No validator, workflow, commit, push, or PR body was changed. The two prose lines were narrowed without changing the required command order or lifecycle semantics. The complete validator sequence was restarted from the beginning and passed.
