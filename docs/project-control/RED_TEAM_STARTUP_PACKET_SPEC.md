...
The version `1.3.2` scope-bound evidence contract requires a structurally valid authoritative live repository name, default-branch name and exact lowercase target SHA without fallback substitution; public-safe requested repository name; normalized remote name and transport; root/remote verification status; normalized remote identifier; PR author identity and account type; approval evidence status; normalized review and permission records; qualifying and disqualified reviewer lists; stable disqualification reasons; PR head branch; check link; workflow identity; event; run head branch; bounded jobs; and numbered step status/conclusion evidence. A valid but different live repository name is contradictory evidence classified by comparison as quarantined, while missing or malformed live identity remains unavailable. The generator and both generated schemas use `1.3.2`; every fixture uses `fixture_version: 1.3.2`. Absolute repository paths and raw remote URLs are never persisted.
